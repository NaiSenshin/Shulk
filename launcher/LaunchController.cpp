...
void LaunchController::decideAccount()
{
    ...
    if (!accounts->anyAccountIsValid()) {
        emitFailed(tr("No account configured. Please connect a Microsoft or Offline account in Settings."));
        return;
    }
...
{
    Q_UNUSED(account);
    emitFailed(reason.isEmpty() ? tr("Account session expired. Please re-authenticate in Settings.") : reason);
    return false;
}
