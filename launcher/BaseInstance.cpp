...
{
    QStringList args = Commandline::splitArgs(settings()->get("JvmArgs").toString());
    bool hasStackSize = false;
    for (const auto& arg : args) {
        if (arg.contains("org.lwjgl.system.stackSize")) {
            hasStackSize = true;
            break;
        }
    }
    if (!hasStackSize) {
        args.append("-Dorg.lwjgl.system.stackSize=4096");
    }
    return args;
}
