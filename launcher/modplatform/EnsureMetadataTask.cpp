...
void EnsureMetadataTask::executeTask()
{
    ...
    connect(version_task.get(), &Task::finished, this, [this] {
        ...
        if (!project_task) {
            m_projectLookupFinished = true;
            finishMetadataUpdates();
            return;
        }

        connect(project_task.get(), &Task::finished, this, [this, project_task] {
            m_projectLookupFinished = true;
...
void EnsureMetadataTask::updateMetadata(ModPlatform::IndexedPack& pack, ModPlatform::IndexedVersion& ver, Resource* resource)
{
    try {
        ...
        // A pack can contain more than one file from the same project. Track
        // each write by its task ID so one completion cannot hide another.
        const QString taskKey = task->getUid().toString(QUuid::WithoutBraces);

        const auto weakTask = task.toWeakRef();
        connect(task.get(), &Task::finished, this, [this, weakTask, resource, taskKey] {
            if (auto finishedTask = weakTask.lock())
                updateMetadataCallback(finishedTask->resolvedSlug(), resource);
            else
                emitFail(resource);
            --m_pendingMetadataUpdates;
            finishMetadataUpdates();
        });

        m_updateMetadataTasks[taskKey] = task;
        ++m_pendingMetadataUpdates;
        task->start();
    } catch (Json::JsonException& e) {
        qDebug() << e.cause();
...
void EnsureMetadataTask::finishMetadataUpdates()
{
    if (!isRunning() || !m_projectLookupFinished || m_pendingMetadataUpdates > 0)
        return;
...
void EnsureMetadataTask::updateMetadataCallback(const QString& slug, Resource* resource)
{
    QDir tmpIndexDir(m_indexDir);
    auto metadata = Metadata::get(tmpIndexDir, slug);
    if (!metadata.isValid()) {
