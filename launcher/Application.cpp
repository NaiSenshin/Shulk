...
bool Application::createSetupWizard()
{
    ...
    if (languageRequired)
        settings()->set("Language", QString("en_US"));

    // In Shulk, we provide a handheld-native onboarding experience rather than blocking on desktop dialogs
    return false;
}
...
void Application::performMainStartupAction()
{
    ...
    if (!m_shulkWindow && !m_mainWindow) {
        showShulkWindow();
        qDebug() << "<> Shulk main window shown.";
    }
...
{
    if (QFile::exists(":/shulk/icons/shulk.png")) {
        return QIcon(":/shulk/icons/shulk.png");
    }
...
ShulkWindow* Application::showShulkWindow()
{
    if (!m_shulkWindow) {
