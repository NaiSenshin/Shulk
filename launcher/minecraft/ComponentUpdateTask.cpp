...
{
    if (d->metadataRefreshTask && d->metadataRefreshTask->isRunning() && !d->metadataRefreshTask->canAbort()) {
        return false;
    }
...
{
    bool aborted = true;
    if (d->metadataRefreshTask && d->metadataRefreshTask->isRunning()) {
        aborted &= d->metadataRefreshTask->abort();
    }
    for (const auto& status : d->remoteLoadStatusList) {
...
void ComponentUpdateTask::executeTask()
{
    ...
    // Imported packs can reference components whose list hashes changed after
    // the application loaded its root metadata index. Refresh that index once
    // before resolving a new profile so the list checksum validators use the
    // current hashes instead of rejecting valid metadata from the CDN.
    if (d->mode == Mode::Resolution && d->netmode == Net::Mode::Online &&
        APPLICATION->settings()->get("MetaRefreshOnLaunch").toBool()) {
        setStatus(tr("Refreshing component metadata"));
        d->metadataRefreshTask = APPLICATION->metadataIndex()->loadTask(Net::Mode::Online, true);
        connect(d->metadataRefreshTask.get(), &Task::succeeded, this, [this]() {
            d->metadataRefreshTask.reset();
            loadComponents();
        });
        connect(d->metadataRefreshTask.get(), &Task::failed, this, [this](const QString& error) {
            d->metadataRefreshTask.reset();
            emitFailed(tr("Could not refresh component metadata:\n%1").arg(error));
        });
        connect(d->metadataRefreshTask.get(), &Task::aborted, this, [this]() {
            d->metadataRefreshTask.reset();
            emitAborted();
        });
        connect(d->metadataRefreshTask.get(), &Task::progress, this, &Task::setProgress);
        connect(d->metadataRefreshTask.get(), &Task::stepProgress, this, &ComponentUpdateTask::propagateStepProgress);
        connect(d->metadataRefreshTask.get(), &Task::status, this, &Task::setStatus);
        connect(d->metadataRefreshTask.get(), &Task::details, this, &Task::setDetails);
        d->metadataRefreshTask->start();
        return;
    }
...
static LoadResult loadComponent(ComponentPtr component, Task::Ptr& loadTask, Net::Mode netmode)
{
    ...
    if (QFile::exists(customPatchFilename)) {
        ...
        if (component->m_version.trimmed().isEmpty()) {
            auto versionList = APPLICATION->metadataIndex()->get(component->m_uid);
            if (versionList) {
                if (!versionList->isLoaded()) {
                    versionList->waitToLoad();
                }
                auto rec = versionList->getRecommended();
                if (!rec && versionList->count() > 0) {
                    rec = versionList->at(0);
                }
                if (rec) {
                    component->m_version = rec->descriptor();
                    component->m_cachedVersion = component->m_version;
                }
            }
        }
